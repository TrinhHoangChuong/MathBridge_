...
public class GiaoVienController {
    ...
    public ResponseEntity<List<BaiTapDTO>> getBaiTapByGiaoVien(@PathVariable String idNv) {
        try {
            System.out.println("=== Controller: getBaiTapByGiaoVien called with idNv: " + idNv + " ===");
            List<BaiTapDTO> baiTaps = baiTapService.getBaiTapByGiaoVien(idNv);
            System.out.println("Controller: Returning " + (baiTaps != null ? baiTaps.size() : 0) + " assignments");
            return ResponseEntity.ok(baiTaps != null ? baiTaps : java.util.Collections.emptyList());
        } catch (Exception e) {
            System.err.println("ERROR in Controller getBaiTapByGiaoVien: " + e.getMessage());
            e.printStackTrace();
